...
public class Solo100RuleTests : CategoryMilestoneRuleTestBase<Solo100Rule>
{
    protected override string Category => TrainingCategories.Solo;
    protected override string AchievementId => AchievementIds.Solo100;
    protected override int Milestone => 100;
    
    protected override Solo100Rule CreateRule(ApplicationDbContext context, Mock<IAchievementService> achievementMock)
    {
        return new Solo100Rule(context, new Lazy<IAchievementService>(() => achievementMock.Object));
    }
    ...
    public async Task GetProgress_ShouldReturnCorrect()
        => await Test_GetProgressAsync(333, Milestone);
}