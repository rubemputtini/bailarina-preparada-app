...
public class Top1MonthlyRuleTests
{
    ...
    public async Task EvaluateAsync_ShouldGrantAchievement_WhenUserIsTop1()
    {
        ...
        achievementMock.Setup(a =>
                a.HasAchievementAsync(_userId, AchievementIds.Top1Monthly, PreviousMonth.Year, PreviousMonth.Month))
            .ReturnsAsync(false);
        
        achievementMock.Setup(a => a.GrantAchievementAsync(_userId, AchievementIds.Top1Monthly))
            .ReturnsAsync(true).Verifiable();
        
        await rule.EvaluateAsync(_userId);
        
        achievementMock.Verify(a => a.GrantAchievementAsync(_userId, AchievementIds.Top1Monthly), Times.Once);
    }
    ...
    public async Task EvaluateAsync_ShouldNotGrantAchievement_WhenAlreadyEarned()
    {
        ...
        achievementMock.Setup(a => a.HasAchievementAsync(_userId, AchievementIds.Top1Monthly, PreviousMonth.Year, PreviousMonth.Month))
            .ReturnsAsync(true);
        
        await rule.EvaluateAsync(_userId);
        
        achievementMock.Verify(a => a.GrantAchievementAsync(_userId, AchievementIds.Top1Monthly), Times.Never);
    }
}