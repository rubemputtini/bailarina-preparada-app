...
public class Top1MonthlyRuleTests
{
    ...
    private (Top1MonthlyRule rule, Mock<IAchievementService> achievementMock, Mock<IRankingService> rankingMock)
        CreateRuleWithMocks(List<RankingResponse> ranking)
    {
        ...
        rankingMock.Setup(r => r.GetRankingAsync(PreviousMonth.Month, PreviousMonth.Year, 1))
            .ReturnsAsync(ranking);
        
        var rule = new Top1MonthlyRule(rankingMock.Object, new Lazy<IAchievementService>(() => achievementMock.Object));
        
        return (rule, achievementMock, rankingMock);
    }

    [Fact]
    public async Task EvaluateAsync_ShouldGrantAchievement_WhenUserIsTop1()
    {
        ...
        achievementMock.Setup(a =>
                a.HasAchievementAsync(_userId, AchievementIds.Top1Monthly, ReferenceFirstDay))
            .ReturnsAsync(false);
        
        achievementMock.Setup(a => a.GrantAchievementAsync(_userId, AchievementIds.Top1Monthly, ReferenceFirstDay))
            .ReturnsAsync(true).Verifiable();
        
        await rule.EvaluateAsync(_userId);
        
        achievementMock.Verify(a => a.GrantAchievementAsync(_userId, AchievementIds.Top1Monthly, ReferenceFirstDay), Times.Once);
    }

    [Fact]
    public async Task EvaluateAsync_ShouldNotGrantAchievement_WhenUserIsNotTop1()
    {
    ...
    public async Task EvaluateAsync_ShouldNotGrantAchievement_WhenAlreadyEarned()
    {
        ...
        achievementMock.Setup(a => a.HasAchievementAsync(_userId, AchievementIds.Top1Monthly, ReferenceFirstDay))
            .ReturnsAsync(true);
        
        await rule.EvaluateAsync(_userId);
        
        achievementMock.Verify(a => a.GrantAchievementAsync(_userId, AchievementIds.Top1Monthly, ReferenceFirstDay), Times.Never);
    }
}